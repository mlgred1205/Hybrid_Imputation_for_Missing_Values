...
write.csv(MV_Final, "AmputedData_MVType_LowMV_20.csv")
...
df4 = data.frame(ProteinID = row.names(df3), data2, 
                 row.names = NULL)
write.csv(df4, "MDA_Sim_Amp_Data_LowMV.csv", row.names = F)


#PLOT MISSING DATA PATTERN AND ADD UP NUMBER OF MISSING VALUES
pdf("MD_Pattern_Sim_Amp_LowMV.pdf")
...
annotate_figure(figure, top = text_grob("MAR KNN Imputation Sim Amp MDA:LowMV", face = "bold", size = 18), 
