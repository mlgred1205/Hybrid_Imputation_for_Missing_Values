...
#SELECT COLUMNS FOR PAIRWISE T-TEST COMPARISONS - EZH2 to IgG
selected_columns_a = IgG
selected_columns_b = EZH2
selected_columns_c = SUZ12

fac <- c(rep("EZH2", 3), rep("IgG", 3)) #JUST REPLACE GROUPS HERE
selected_columns = c("ProteinID", selected_columns_a, selected_columns_b)
...
for (i in seq(1, maxiterations))  {
  ...
  nrow(df_merge)
  head(df_merge)
  write.csv(df_merge, paste0("Imp_Post_Hybrid_QRILC_KNN_SFI_EZH2toIgG", i, ".csv"), row.names = F)
  write.csv(df.threshold, "Model_Selector_Post_Hybrid_SFI_EZH2toIgG.csv", row.names = F)
  
  #VISUALIZE DATA BEFORE AND AFTER IMPUTATIONS AS HISTOGRAM
  pdf("Intensity_Distribution_Hybrid_Imputation_QRILC_KNN_SFI_EZH2toIgG.pdf")
  ...
  ggplot() + 
    ...
    xlab("Distribution Post Hybrid Imputation SFI: EZH2 IP") +
    ylab("Treatments") +
    theme_bw() +
    theme(axis.title=element_text(size=14,  family="Helvetica", face = "bold"))
  ...
  ggplot() + 
    ...
    xlab("Distribution Overlay Hybrid Imputation SFI: EZH2 IP") +
    ylab("Treatments") +
    theme_bw() +
    theme(axis.title=element_text(size=14,  family="Helvetica", face = "bold"))
  ...
  pdf("PCA_Scree_Post_Hybrid_SFI_EZH2toIgG.pdf")
  ...
  pdf("BoxPlots_Post_Hybrid_SFI_EZH2toIgG.pdf")
  ...
  ggplot(data_box3, aes(x = ind, y = values)) +
    ...
    labs(title = "LFQ Intensity Values:\nPost Hybrid EZH2 IP", y = "LFQ Intensity",
         x = "Treatment") +
    theme_bw() +
  ...
  pdf("Violin_Box_Post_Hybrid_SFI_EZH2toIgG.pdf")
  ...
  pdf("Pvalues__Post_Hybrid_SFI_EZH2toIgG.pdf")
  ...
  pdf("QValues_Post_Hybrid_SFI_EZH2toIgG.pdf")
  plot(qobj)
  dev.off()
  ...
  write.csv(output_data[order(output_data$qvalue),], 
            paste0("DE_Ordered_Hybrid_Imputation_SFI_EZH2toIgG_", i,".csv"), 
            row.names = F)
...
write.csv(big_data, "Long_data_qvalue_logFC_Hybrid_SFI_EZH2_IP.csv", row.names = F)
...
write.csv(df_combined, "Combined_logFC_qvalue_pvalue_Hybrid_SFI_EZH2toIgG.csv", row.names = F)

pdf("Boxplots_Variance_Hybrid_SFI_EZH2_IP.pdf")
...
annotate_figure(figure, top = text_grob("Hybrid Imputation SFI EZH2 IP", face = "bold", size = 18), 
                fig.lab = "Fig. S8", fig.lab.face = "bold")
...
options(scipen = 0)
my_data1 <-read.csv("Combined_logFC_qvalue_pvalue_Hybrid_SFI_EZH2toIgG.csv", 
                    sep = ",")
head(my_data1)
...
pdf("Volcano_Hybrid_Imp_EZH2toIgG_PRC2_Inset.pdf", width = 8, height = 8)
...
g + labs(x = "log2 Fold Change (EZH2/IgG)", y = "-log 10 qvalue",
         main = "EZH2 IP Spectral Counts") + 
  xlim(3, 11) +
  ylim(4, 6) +
